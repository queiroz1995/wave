...
export const RouletteMode = () => {
    ...
    // Estatísticas baseadas APENAS no histórico de sorteios oficiais
    const stats = useMemo(() => {
    ...
    // Último número sorteado para mostrar no centro durante o "Sorteando"
    ...
    return (
        <Card className="bg-card/80 backdrop-blur-sm overflow-hidden border-primary/20 h-full">
            <CardContent className="p-6 space-y-6">
                ...
                {/* BARRA DE ÚLTIMOS SORTEIOS (OFICIAIS) */}
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase flex items-center gap-1">
                            <History className="h-3 w-3" /> Histórico de Sorteios
                        </p>
                    </div>
                ...
                <div className="flex flex-col items-center justify-center space-y-4 pt-4 border-t border-white/5">
                    <div className="relative w-40 h-40 flex items-center justify-center">
                        ...
                        <div className="text-center z-10">
                            {!isBettingOpen && lastDrawnNumber !== undefined ? (
                                <div className="animate-in zoom-in duration-300">
                                    <div className={cn(
                                        "w-16 h-16 rounded-full flex items-center justify-center text-4xl font-black text-white mx-auto shadow-lg mb-1",
                                        getDigitColor(lastDrawnNumber)
                                    )}>
                                        {lastDrawnNumber}
                                    </div>
                                    <p className="text-[10px] font-black text-yellow-500 uppercase animate-pulse">Sorteado!</p>
                                </div>
                            ) : (
                                <>
                ...
                <div className="space-y-4 pt-4 border-t border-white/5">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 text-center bg-muted/30 rounded-xl py-2 border border-white/5 shadow-inner">
                            <p className="text-[10px] font-bold text-muted-foreground uppercase mb-1">Valor por Número</p>
                ...
                <div className="text-center pt-2">
                    {!isConnected ? (
                        <div className="p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
                            <p className="text-[10px] text-yellow-500 font-black uppercase tracking-wider">Modo Demonstração</p>
                            <p className="text-[10px] text-muted-foreground mt-1">Conecte sua conta para processar apostas.</p>
                        </div>
                    ) : (
                        <div className="p-3 rounded-xl bg-green-500/10 border border-green-500/20 flex items-center justify-center gap-2">
                            ...
                            <p className="text-[10px] text-green-500 font-black uppercase tracking-wider">Sistema de Apostas Ativo</p>
                        </div>
                    )}
                </div>
            </CardContent>
