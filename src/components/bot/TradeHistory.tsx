        ...
        <Card className="h-full flex flex-col bg-card/80 backdrop-blur-sm">
            ...
            <CardContent className="flex-grow p-2">
                <div ref={logContainerRef} className="h-full bg-muted/30 rounded-md p-2 overflow-y-auto space-y-1 custom-scrollbar">
                    {signals.length > 0 ? (signals.map((signal, index) => <SignalItem key={index} signal={signal} />)) : (<div className="flex items-center justify-center h-full text-muted-foreground">Nenhum resultado registrado.</div>)}
                </div>
            </CardContent>
        </Card>
    );
