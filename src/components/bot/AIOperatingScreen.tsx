...
export const AIOperatingScreen = () => {
    ...
    return (
        <div className="w-full max-w-md mx-auto space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700 px-2 sm:px-0">
                ...
                <div className="grid grid-cols-4 gap-1">
                    {attackModes.map((mode) => (
                        <button
                            ...
                            className={cn(
                                "flex flex-col items-center justify-center py-1.5 sm:py-2 rounded-xl sm:rounded-2xl transition-all duration-300",
                        ...
                        >
                            <span className="text-[8px] sm:text-[9px] font-black tracking-widest">{mode.label}</span>
                            <span className="hidden sm:inline text-[7px] font-bold opacity-70 uppercase truncate w-full px-1">{mode.desc}</span>
                        </button>
                    ))}
                </div>
            </div>
            ...
            {isBotRunning && !isStudying && (
                <div className="bg-green-500/10 border-2 border-green-500/20 rounded-2xl p-3 sm:p-4 flex items-center justify-between shadow-lg shadow-green-500/5">
                    <div className="flex items-center gap-3">
                        <Swords className="h-5 w-5 sm:h-6 sm:w-6 text-green-500 animate-pulse" />
                        <div>
                            <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-green-600">NÚCLEO MULTIMODAL ATIVO</p>
                            <p className="text-[10px] sm:text-xs font-bold text-green-700">Sincronizando {attackMode.length} estratégias...</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isStudying && (
                <div className="bg-blue-500/10 border-2 border-blue-500/50 rounded-2xl p-3 sm:p-4 flex items-center justify-between animate-pulse">
                    <div className="flex items-center gap-3">
                        <BrainCircuit className="h-5 w-5 sm:h-6 sm:w-6 text-blue-500" />
                        <div>
                            <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-blue-500">Aguardando Confirmação</p>
                            <p className="text-[10px] sm:text-xs font-bold text-blue-700">Validando tendência... ({studyTicksCount}/5)</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <Card className="glass-panel border-none shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] overflow-hidden rounded-[2rem] sm:rounded-[3rem]">
                <CardContent className="p-6 sm:p-10 space-y-6 sm:space-y-8">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3 sm:gap-4">
                            <div className="h-10 w-10 sm:h-14 sm:w-14 bg-primary/10 rounded-xl sm:rounded-2xl overflow-hidden border-2 border-white/50 shadow-sm">
                                {selectedAIInfo?.image ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center"><Bot className="h-5 w-5 sm:h-7 sm:w-7 text-primary" /></div>
                                )}
                            </div>
                            <div>
                                <h2 className="text-lg sm:text-2xl font-black uppercase tracking-tighter italic flex items-center gap-1.5 sm:gap-2">
                                    {selectedAIInfo?.name || 'WAVE AI'} <Zap className="h-4 w-4 sm:h-5 sm:w-5 text-blue-500 fill-current" />
                                </h2>
                                <Badge variant="secondary" className="text-[8px] sm:text-[9px] font-black px-2 py-0 uppercase tracking-widest bg-blue-500/10 text-blue-600 border-none">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2">
                            <Button 
                                variant="ghost" 
                                size="icon" 
                                className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl hover:bg-gray-100"
                            ...
                            <SettingsSheet trigger={
                                <Button variant="ghost" size="icon" className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl hover:bg-blue-50 hover:text-blue-500">
                                    <FileSpreadsheet className="h-5 w-5 sm:h-6 sm:w-6" />
                                </Button>
                            } />
                            <Button variant="ghost" size="icon" className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl hover:bg-red-50 hover:text-red-500" onClick={exitToSelection}>
                    ...
                    <Button 
                        ...
                        className={cn(
                            "w-full h-16 sm:h-20 rounded-[1.5rem] sm:rounded-[2rem] text-lg sm:text-xl font-black uppercase tracking-[0.2em] transition-all duration-500 shadow-2xl",
                    ...
                    <div className="bg-gray-50/50 border border-gray-100 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 flex items-center justify-between">
                        <div className="flex items-center gap-3 sm:gap-4">
                            <div className="bg-white p-2 sm:p-2.5 rounded-xl shadow-sm"><Target className="h-4 w-4 sm:h-5 sm:w-5 text-primary" /></div>
                            <div>
                                <p className="text-[8px] sm:text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-60">Carteira WAVE</p>
                                <p className="text-lg sm:text-xl font-black tracking-tight">{accountBalance?.toFixed(2) || '0.00'} <span className="text-[10px] sm:text-xs font-bold opacity-40">USD</span></p>
                            </div>
                        </div>
                        <Button variant="ghost" size="icon" className="h-9 w-9 sm:h-10 sm:w-10 rounded-xl" onClick={() => handleConnect(accountType, currentToken)}>
            ...
            <ScrollArea className="h-40 sm:h-48 px-2">
                <table className="w-full text-[9px] sm:text-[10px] font-bold">
                    <tbody className="divide-y divide-gray-50">
                        {signals.length > 0 ? signals.map((s: any) => {
                            ...
                            return (
                                <tr key={s.id}>
                                    ...
                                    <td className="py-2.5 sm:py-3">
                                        <span className={cn("px-1.5 py-0.5 rounded-md text-[8px] sm:text-[9px] font-black uppercase", label.color)}>
                                            {label.text}
                                        </span>
                                    </td>
                            ...
                            <tr><td colSpan={4} className="py-12 sm:py-16 text-center text-[8px] sm:text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em]">Scanner_WAVE_Active</td></tr>
                        )}
                    </tbody>
                </table>
            </ScrollArea>
