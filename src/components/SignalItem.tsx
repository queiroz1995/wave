...
const SignalItem: React.FC<SignalItemProps> = ({ signal }) => {
    const hasResult = !!signal.result;
    const config = hasResult ? resultConfig[signal.result!] : null;
    ...
    const profitColor = signal.profit && signal.profit > 0 ? 'text-green-400' : 'text-red-400';
    const profitSign = signal.profit && signal.profit >= 0 ? '+' : '';
